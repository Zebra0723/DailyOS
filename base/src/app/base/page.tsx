    ...
    <div className="grid gap-6">
      ...
      <section className={card}>
        <div className="mb-3 flex items-center gap-2">
          <Database className="size-4 text-[#21577d]" />
        ...
        {missing.length === 0 ? (
          ...
          <div className="rounded-xl border border-[#f0c4bd] bg-[#fbe9e7] p-3 text-sm">
            <p className="font-semibold text-[#21577d]">
              {missing.length} table{missing.length === 1 ? "" : "s"} missing:{" "}
              {missing.map((m) => m.table).join(", ")}.
            </p>
            <Link href="/base/sql" className="mt-2 inline-flex items-center gap-1.5 rounded-lg bg-[#21577d] px-3 py-1.5 text-xs font-semibold text-white">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {present.map((r) => (
            <Link key={r.table} href={`/base/tables?t=${r.table}`} className={`${card} transition-colors hover:border-[#21577d]`}>
              <div className="text-xl font-bold text-[#21577d]">{r.count}</div>
      ...
      <div className="flex flex-wrap gap-2">
        <Link href="/base/tables" className="inline-flex items-center gap-2 rounded-lg border border-[#e6ded2] bg-[#fffdf9] px-4 py-2 text-sm font-semibold hover:border-[#21577d]">
          <Table2 className="size-4 text-[#21577d]" /> Browse tables
        </Link>
        <Link href="/base/sql" className="inline-flex items-center gap-2 rounded-lg border border-[#e6ded2] bg-[#fffdf9] px-4 py-2 text-sm font-semibold hover:border-[#21577d]">
          <Terminal className="size-4 text-[#21577d]" /> SQL console
        </Link>
      </div>
