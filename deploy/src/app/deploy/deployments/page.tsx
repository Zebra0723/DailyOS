              ...
              return (
                <div key={d.uid} className="rounded-xl border border-[#e6ded2] bg-[#fffdf9] p-3">
                  <div className="flex items-center gap-2">
                    ...
                    {d.target === "production" && <span className="rounded-md bg-[#21577d] px-2 py-0.5 text-[11px] font-bold text-white">PROD</span>}
                    <span className="ml-auto text-xs text-[#8a8073]">{d.created ? new Date(d.created).toLocaleString() : ""}</span>
                  </div>
                  <Link href={`/deploy/${d.uid}`} className="mt-1.5 block truncate text-sm font-medium hover:text-[#21577d] hover:underline">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-[#8a8073]">
                    ...
                    {d.url && (
                      <a href={`https://${d.url}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[#21577d] hover:underline">
