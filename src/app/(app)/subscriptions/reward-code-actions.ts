...
export async function redeemRewardCode(raw: string): Promise<RedeemResult> {
  ...
  try {
    const { data: existing } = await admin
      .from("reward_codes")
      .select("code,kind,percent,plan_tier,plan_days,used,created_at,recipient_id")
      .eq("code", code)
      .maybeSingle();

    if (!existing) return { ok: false, reason: "invalid" };
    if (existing.recipient_id && existing.recipient_id !== user.id)
    ...
    const { error } = await admin
      ...
      .update({
        ...
        recipient_id: existing.recipient_id ?? user.id,
      })
      .eq("code", code)
      .eq("used", false);
